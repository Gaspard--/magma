...
namespace magma
{
  template<class Deleter = Deleter<vk::BufferView>>
  ...
  {
    return BufferView<>(Deleter<vk::BufferView>{magma::Device<claws::no_delete>(*this)}, vk::Device::createBufferView({flags, buffer, format, offset, size}));
  }
};
