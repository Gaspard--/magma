...
public:
  ...
  VulkanHandler(VulkanHandler const &) = delete;
  VulkanHandler(VulkanHandler &&) = delete;

  VulkanHandler(std::vector<char const *> &&extensions = {})
...
public:
  ...
  VulkanDeviceHandler(vk::PhysicalDevice physicalDevice, unsigned int queueId, std::vector<char const *> extensions = {})
    : device([](vk::PhysicalDevice physicalDevice, unsigned int queueId, std::vector<char const *> extensions)
	     {
	       float priority(1.0f);
	       vk::DeviceQueueCreateInfo deviceQueueCreateInfo{{}, queueId, 1, &priority};
	       vk::DeviceCreateInfo deviceCreateInfo{{}, 1, &deviceQueueCreateInfo, 0, nullptr, static_cast<unsigned>(extensions.size()), extensions.data()};

	       return physicalDevice.createDevice(deviceCreateInfo);
	     }(physicalDevice, queueId, extensions))
  {
  }
